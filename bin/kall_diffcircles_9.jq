...
import module namespace ns = "http://dgms.io/contrib/models/minlplib/kall_diffcircles_9";
...
let $input := {
    ...
    "x18": a:variable({ name: "x18", bounds: [0.6, 17.4] }),
    "x19": a:variable({ name: "x19", bounds: [0.6, 3.4] }),
    "x20": a:variable({ name: "x20", bounds: [0.0, 18.0] }),
    "x21": a:variable({ name: "x21", bounds: [0.0, 4.0] }),
    "x22": a:variable({ name: "x22", bounds: [0.0, 72.0] })
} return {
    "obj": a:minimize({
        model: ns:kall_diffcircles_9#1,
        input: $input,
